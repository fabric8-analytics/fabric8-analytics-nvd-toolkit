...
def __has_uppercase(tagged: list, pos: int) -> bool:
    """Return whether the word in `tagged` contains uppercase letter.

    :param tagged: list of tuples (word, tag)
    :param pos: int, position of the word in `tagged`
    """
    word, _ = tagged[pos]  # type: str

    return any([w.isupper() for w in word])


def __is_alnum(tagged: list, pos: int) -> bool:
...
def __vendor_product_match(tagged: list, pos: int) -> bool:
    """Return whether the given word matches vendor or product."""
    raise NotImplementedError


def __ver_follows(tagged: list, pos: int) -> bool:
    """Return whether the given word is followed by a version string."""
    version_tag = '<VERSION>'
    ver_pos = [
        p for p, (_, t) in enumerate(tagged)
        if t == version_tag
    ]
...
def __ver_pos(tagged: list, pos: int) -> typing.Union[int, None]:
    ...
    version_tag = '<VERSION>'
    ver_pos = [
        p - pos for p, (_, t) in enumerate(tagged)
        if t == version_tag
    ]
...
def __word_len(tagged: list, pos: int, cmp=None, limit=3) -> bool:
